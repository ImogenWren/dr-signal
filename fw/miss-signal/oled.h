...
void led_stats_screen() {
  ...
  dtostrf(current_output, 5, 2, val_buffer);
  ...
  display.setCursor(COL_0 + 2, ROW_1);
  display.print(F("mA Output "));
  display.setCursor(COL_0+5, ROW_3);
  display.print(val_buffer);
  display.print(F(" mA"));
  ...
  display.print(F(" V output "));
  display.setCursor(COL_2, ROW_3 - 5);
  display.print(F(" Coming"));
  display.setCursor(COL_2, ROW_4 - 5);
...
void update_oled() {  // char *Vexp, char *Iexp, char *Vsbc, char *Isbc, char *sbcTrig, char *extTrig, char *expTrig, char *sbcReboot, char *expReboot) {
  ...
  display.fillRoundRect(0, 0, 69, 9, 2, WHITE);  // x, y, w, h, colour // full bar
  // display.setFont(&FreeMonoBold9pt7b);
  display.setCursor(2, 1);  //(x,y) (COL, ROW)
  display.print(F("miss signal"));
